...
class Momesso_Default_Form_Clientes_Fisica extends EasyBib_Form {
        ...
        $nome = $this->createElement('text', 'fantasia', array('label' => 'Nome: '))
                        ->setRequired(true)
                        ->addFilter('StripTags')
                        ->addFilter('stringTrim')
                        ->setAttrib('placeholder', 'Nome do Cliente')
                        ->setAttrib('maxlength', 100)
                        ->setAttrib('size', 100);
        $this->addElement($nome);

        $cpf = $this->createElement('text', 'documento', array('label' => 'CPF ( Somente números ) : '))
                     ->setRequired(true)
                     ->addFilter('StripTags')
                     ->addFilter('stringTrim')
                     ->addValidator(new Zend_Validate_Db_NoRecordExists('mod_clientes', 'documento'))
        ...
        $email = $this->createElement('text', 'email', array('label' => 'E-mail: '))
                ...
                ->addValidator('Db_NoRecordExists', false, array('table' => 'mod_clientes', 'field' => 'email'))
                ->addFilter('StripTags')
                ->addFilter('stringTrim')
                ->setAttrib('maxlength', 50)
                ->setAttrib('size', 50);
        ...
        $password = $this->createElement('text', 'senha', array('label' => 'Senha: ( até 8 caracteres ) '))
                     ->setRequired(TRUE)
                     ->addFilter('StripTags')
                     ->addFilter('stringTrim')
