...
class Admin_GrupoController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $this->Model 			= new Model_ProdutoGrupo();
        $this->Form 			= new Momesso_Admin_Form_Produtos_Grupo();
    ...
    public function indexAction() {
    	...
    	$this->view->Data = $this->Model->getGrupos();
    }
    		...
    		if ($this->Form->isValid($dados)) {
    			...
    			try {
                    ...
                    if ($ext[0]) {
                        ...
                        $adapter->addFilter('Rename', array('target' => 'default/uploads/grupos/'.$arquivo, 'overwrite' => true));
                    
                        if ($adapter->receive()) {
                            $this->Model->save(array('imagem' => $arquivo),$idInsert);
                            $img = WideImage::load(getcwd() . '/default/uploads/grupos/'.$arquivo);
                            $img->saveToFile(getcwd() . '/default/uploads/grupos/'.$arquivo);
                        }
                    }
	...
	public function editAction(){
        ...
        if ($this->_request->isPost()) {
            ...
            if (!empty($r->imagem)) {
                ...
                $fileCaminho = $baseUrl.'/default/uploads/grupos/'.$r->imagem;
    ...
    public function deleteAction(){
        ...
        if (!empty($r['imagem'])) {
            $file = getcwd() . '/default/uploads/grupos/'.$r['imagem'];
            if (file_exists($file)) {
                unlink($file);
            }
        }
